...
| Servicio corriendo en testnet | `GET /v1/live` y `GET /v1/health` (contratos = `deployments.json`, `network: testnet`, admin `GBLS7PL5…YC2P`) | https://raiz-relayer.fly.dev · `raiz-relayer/docs/evidencia/deploy_fly.md` · [`capturas/d1_relayer_health_2026-09-12.png`](capturas/d1_relayer_health_2026-09-12.png) | **Desplegado 2026-09-06** (Fly, región `iad`, 1 máquina, `/v1/health` 200 con los 6 contratos; seguía vivo el 12-sep con uptime ≈ 6 días) |
...
| App migrada al relayer (faucet, comercio, residente, vault) | Cliente HTTP `android/app/src/main/java/com/raiz/app/data/relayer/RelayerClient.kt` + test unitario de mapeo de errores (`RelayerClientTest`, MockEngine); ViewModels sin clave admin | Este repo, `main` (PR #1 mergeado el 2026-09-06, commit `920c2fe`) | **En `main`** |
| APK release sin secretos | `app-release.apk` 0.2.0 (`versionCode 2`), firmado con la clave debug de Android para que sea instalable (H9; keystore propio en WP4) | **https://github.com/JuanWimmin/Protocolo_Raiz/releases/download/v0.2.0/raiz-0.2.0-relayer.apk** (Release `v0.2.0`, notas y sha256 en https://github.com/JuanWimmin/Protocolo_Raiz/releases/tag/v0.2.0) | **Publicado 2026-09-12** (`74ec7529…`, firmado con clave debug) |
...
| Regresión en dispositivo físico | Checklist de 4 flujos (+ negativo sin relayer) en Motorola G04 con capturas y hashes | [`regresion_dispositivo.md`](regresion_dispositivo.md) · `capturas/` | **Aprobada 2026-09-06** en Motorola G04 con el APK `74ec7529…` — 4 flujos vía relayer + caso negativo sin relayer, con capturas y hashes |
